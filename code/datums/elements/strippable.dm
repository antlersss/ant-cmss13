...
/datum/strippable_item/proc/try_unequip(atom/source, mob/user)
	...
	if (ishuman(source))
		var/mob/living/carbon/human/sourcehuman = source
		if(MODE_HAS_MODIFIER(/datum/gamemode_modifier/disable_stripdrag_enemy) && (sourcehuman.stat == DEAD || sourcehuman.health < sourcehuman.health_threshold_crit + sourcehuman.reagent_crit_threshold_modifier) && !sourcehuman.get_target_lock(user.faction_group))
